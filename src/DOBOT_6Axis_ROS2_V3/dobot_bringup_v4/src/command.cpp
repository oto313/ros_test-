...
void CRCommanderRos2::recvTask()
{
    ...
    while (is_running_)
    {
        ...
        else
        {
            try
            {
                 std::cout << "realtime connect" << std::endl;
        ...
        if (!dash_board_tcp_->isConnect())
        {
            try
            {
                std::cout << "dashboard connect" << std::endl;
                               ...
                               std::vector<std::string> &result)
{
    try
    {
        ...
        std::string feedback(recv_ptr);
        std::cout << "tcp recv feedback : " << feedback << std::endl; // FIXME parse the buf may be better
    }
