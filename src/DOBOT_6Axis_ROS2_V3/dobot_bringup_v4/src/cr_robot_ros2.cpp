...
#include <chrono>
#include <thread>
CRRobotRos2::CRRobotRos2(std::string name) : rclcpp::Node("dobot_bringup_ros2"), followPath(*this){
    RCLCPP_INFO(this->get_logger(), "initializing");
         this->action_server_ = rclcpp_action::create_server<control_msgs::action::FollowJointTrajectory>(
      this,
      "/arm_controller/follow_joint_trajectory",
    std::bind(&CRRobotRos2::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
    std::bind(&CRRobotRos2::handle_cancel, this, std::placeholders::_1),
    std::bind(&CRRobotRos2::handle_accepted, this, std::placeholders::_1));

    gripper_action_server_ = rclcpp_action::create_server<control_msgs::action::GripperCommand>(
      this,
      "/gripper_controller/gripper_cmd",
      std::bind(&CRRobotRos2::gripper_handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&CRRobotRos2::gripper_handle_cancel, this, std::placeholders::_1),
      std::bind(&CRRobotRos2::gripper_handle_accepted, this, std::placeholders::_1));
};

 rclcpp_action::GoalResponse CRRobotRos2::gripper_handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const GripperCommand::Goal> goal)
  {
    RCLCPP_INFO(this->get_logger(), "Received goal request with position %f and max effort %f", goal->command.position, goal->command.max_effort);
    // Accept all goals for now
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
  }

  rclcpp_action::CancelResponse CRRobotRos2::gripper_handle_cancel(const std::shared_ptr<GoalHandleGripperCommand> goal_handle)
  {
    RCLCPP_INFO(this->get_logger(), "Received request to cancel goal");
    return rclcpp_action::CancelResponse::ACCEPT;
  }

  void CRRobotRos2::gripper_handle_accepted(const std::shared_ptr<GoalHandleGripperCommand> goal_handle)
  {
    std::thread{std::bind(&CRRobotRos2::gripper_execute, this, goal_handle)}.detach();
  }

  void CRRobotRos2::gripper_execute(const std::shared_ptr<GoalHandleGripperCommand> goal_handle)
  {

    // Example feedback loop for gripper movement
    ...
    if(position < 0.001){
        ...
        state = GripperState::Opened;
         ...
         RCLCPP_INFO(this->get_logger(), "Almost opening gripper");
        state = GripperState::AlmostOpened;
    } else if(position < 0.012){
         RCLCPP_INFO(this->get_logger(), "Almost closing gripper");
        state = GripperState::AlmostClosed;
    } else{
         RCLCPP_INFO(this->get_logger(), "Closing gripper");
        state = GripperState::Closed;
    }
    auto request = std::make_shared<dobot_msgs_v4::srv::DO::Request>();
    int32_t a=0;
    request ->index = 1;
    request ->status = static_cast<int>(state) & 0b01;
    commander_->callRosService(parseTool::parserDORequest2String(request), a);
    request ->index = 2;
    request ->status = (static_cast<int>(state) & 0b10) >> 1;
    commander_->callRosService(parseTool::parserDORequest2String(request), a);
...
double CRRobotRos2::getGripperPosition(){
...
void CRRobotRos2::init()
{
    ...
    RCLCPP_INFO(this->get_logger(), "Service: %s", serviceEnableRobot.c_str()); 
    ...
    commander_->init();
    followPath.commander_ = commander_;
...
bool CRRobotRos2::ServoP(const std::shared_ptr<dobot_msgs_v4::srv::ServoP::Request> request, const std::shared_ptr<dobot_msgs_v4::srv::ServoP::Response> response)
{
    ...
    rclcpp_action::GoalResponse CRRobotRos2::handle_goal(
    ...
    rclcpp_action::CancelResponse CRRobotRos2::handle_cancel(
    ...
    void CRRobotRos2::handle_accepted(const std::shared_ptr<GoalHandleFollowJointTrajectory> goal_handle)
    {
        using namespace std::placeholders;
        std::thread{std::bind(&CRRobotRos2::execute, this, _1), goal_handle}.detach();
    }

    void CRRobotRos2::execute(const std::shared_ptr<GoalHandleFollowJointTrajectory> goal_handle)
    {
        followPath.execute(goal_handle);
    }
