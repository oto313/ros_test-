...
int main(int argc, char *argv[])
{
  ...
  std::string b = "_robot/follow_joint_trajectory";
  std::string ss = z + a + b;
  auto robot = std::make_shared<CRRobotRos2>("/nova2_group_controller/follow_joint_trajectory");
  ...
  joint_state_msg.position.push_back(0.0);
  joint_state_msg.name.push_back(std::string("right_joint"));
  ...
  while (rclcpp::ok())
  {
    ...
    joint_state_msg.position[6] = robot->getGripperPosition();
