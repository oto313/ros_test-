...
def generate_launch_description():
    xacro_file = os.path.join(get_package_share_directory("dobot_rviz"),"urdf/nova2_robot_with_pgc-50-35.xacro")
    robot_description_raw = xacro.process_file(xacro_file).toxml()
    robot_description = {"robot_description": robot_description_raw}

    # Get the launch directory
    bringup_dir = get_package_share_directory('dobot_rviz')
    ...
    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        'rviz_config_file',
        default_value=os.path.join(bringup_dir, 'rviz', 'gripper.rviz'),
